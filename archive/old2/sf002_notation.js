...
var joinPiece = false;
// PIECE ID ----------------------- >
//Get URL variables here
//Also launch join piece
// var urlArgs = getUrlVars(
//   function(vars) {
//     if (vars.joinpiece == 'true') {
//       joinPiece = true;
//     }
//   }
// );
var urlArgs = getUrlVars();
var pieceID = urlArgs.id;
var pieceType = 'sf002';
...
function mkCtrlPanel_generateParts() {
  ...
  genPartsBtn.addEventListener("click", function() {
    //Button will perform different functions for joinPiece
    if (!joinPiece) {
      ...
      // JOIN PIECE
      var t_absStartTime = parseFloat(urlArgs.absStartTime);
      var t_timeAdj = parseFloat(urlArgs.timeAdjustment);
      var t_now = new Date(ts.now());
      var t_time = t_now.getTime();
      var t_startTime = t_now - t_absStartTime;
      timeAdjustment = t_timeAdj;
      clockAdj = t_startTime;
      // GENERATE PART SCORES W/STATIC ELEMENTS ---- >
      //Based on which player checkboxes are chosen
      cbs.forEach((it, ix) => {
        if (it[0].checked) {
